...
echo "Running EdgeAdder under NIFA (degree) for 5 datasets x 1 backbone (gcn) x 10 seeds..."
echo "CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
echo "DRY_RUN=$DRY_RUN"
echo

# ---------------------------------------------
# Tuned best_overall.json paths (GCN templates)
# We swap /gcn/ -> /${encoder}/
# ---------------------------------------------
...
#   [bail]="best_overall_json/vanilla_nifa/gcn/bail.json"
#   [pokec_z]="best_overall_json/vanilla_nifa/gcn/pokec_z.json"
#   [pokec_n]="best_overall_json/vanilla_nifa/gcn/pokec_n.json"
#   [nba]="best_overall_json/vanilla_nifa/gcn/nba.json"
#   [german]="best_overall_json/vanilla_nifa/gcn/german.json"
# )

declare -A best_ea_paths=(
  [bail]="best_overall_json/ea_obj/gcn/bail.json"
  [pokec_z]="best_overall_json/ea_obj/gcn/pokec_z.json"
  [pokec_n]="best_overall_json/ea_obj/gcn/pokec_n.json"
  [nba]="best_overall_json/ea_obj/gcn/nba.json"
  [german]="best_overall_json/ea_obj/gcn/german.json"
)
...
allow_gcn_fallback=1

log_dir="logs/scaled_use_tuned_hp/tuned_ea/nifa_degree/"
...
# -----------------------------
# Loops: gcn x 5 datasets
# -----------------------------
for encoder in gcn; do
  echo
  echo "================ ENCODER: ${encoder^^} ================"

  for dataset in nba bail german pokec_z pokec_n; do
    echo
    echo "============ ${dataset^^} ============="
    ...
    best_vanilla_path="${vanilla_tmpl/\/gcn\//\/${encoder}\/}"
    # best_nifa_path="${nifa_tmpl/\/gcn\//\/${encoder}\/}"
    best_ea_path="${ea_tmpl/\/gcn\//\/${encoder}\/}"

    # -------- existence check + fallback to GCN --------
    if [[ ! -f "$best_vanilla_path" ]]; then
      echo "⚠️  Warning: vanilla best_overall.json not found at: $best_vanilla_path"
      if [[ "$allow_gcn_fallback" -eq 1 && -f "$vanilla_tmpl" ]]; then
        echo "    -> fallback to GCN tuned file: $vanilla_tmpl"
    ...
    #   if [[ "$allow_gcn_fallback" -eq 1 && -f "$nifa_tmpl" ]]; then
    #     echo "    -> fallback to GCN tuned file: $nifa_tmpl"
    ...
    if [[ ! -f "$best_ea_path" ]]; then
      ...
      if [[ "$allow_gcn_fallback" -eq 1 && -f "$ea_tmpl" ]]; then
        echo "    -> fallback to GCN tuned file: $ea_tmpl"

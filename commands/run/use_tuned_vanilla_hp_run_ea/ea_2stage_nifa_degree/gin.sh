...
echo "Running EdgeAdder under NIFA (degree) for 5 datasets x 1 backbone (gin) x 10 seeds..."
echo "CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
echo "DRY_RUN=$DRY_RUN"
echo

# ---------------------------------------------
# Tuned best_overall.json paths (gin templates)
# We swap /gin/ -> /${encoder}/
# ---------------------------------------------
...
for encoder in gin; do
  ...
  for dataset in pokec_z pokec_n nba bail german; do
    ...
    # best_nifa_path="${nifa_tmpl/\/gin\//\/${encoder}\/}"
    best_ea_path="${ea_tmpl/\/gin\//\/${encoder}\/}"

    # -------- existence check + fallback to gin --------
    ...
    #   if [[ "$allow_gin_fallback" -eq 1 && -f "$nifa_tmpl" ]]; then
    #     echo "    -> fallback to gin tuned file: $nifa_tmpl"

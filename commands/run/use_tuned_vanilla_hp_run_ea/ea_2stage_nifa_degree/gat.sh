...
echo "Running EdgeAdder under NIFA (degree) for 5 datasets x 1 backbone (gat) x 10 seeds..."
echo "CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
echo "DRY_RUN=$DRY_RUN"
echo

# ---------------------------------------------
# Tuned best_overall.json paths (gat templates)
# We swap /gat/ -> /${encoder}/
# ---------------------------------------------
...
#   [bail]="best_overall_json/vanilla_nifa/gat/bail.json"
#   [pokec_z]="best_overall_json/vanilla_nifa/gat/pokec_z.json"
#   [pokec_n]="best_overall_json/vanilla_nifa/gat/pokec_n.json"
#   [nba]="best_overall_json/vanilla_nifa/gat/nba.json"
#   [german]="best_overall_json/vanilla_nifa/gat/german.json"
# )

declare -A best_ea_paths=(
  [bail]="best_overall_json/ea_obj/gat/bail.json"
  [pokec_z]="best_overall_json/ea_obj/gat/pokec_z.json"
  [pokec_n]="best_overall_json/ea_obj/gat/pokec_n.json"
  [nba]="best_overall_json/ea_obj/gat/nba.json"
  [german]="best_overall_json/ea_obj/gat/german.json"
)
...
# -----------------------------
# Loops: gat x 5 datasets
# -----------------------------
for encoder in gat; do
  echo
  ...
  for dataset in pokec_z pokec_n nba bail german; do
    ...
    best_vanilla_path="${vanilla_tmpl/\/gat\//\/${encoder}\/}"
    # best_nifa_path="${nifa_tmpl/\/gat\//\/${encoder}\/}"
    best_ea_path="${ea_tmpl/\/gat\//\/${encoder}\/}"

    # -------- existence check + fallback to gat --------
    if [[ ! -f "$best_vanilla_path" ]]; then
      echo "⚠️  Warning: vanilla best_overall.json not found at: $best_vanilla_path"
      if [[ "$allow_gat_fallback" -eq 1 && -f "$vanilla_tmpl" ]]; then
        echo "    -> fallback to gat tuned file: $vanilla_tmpl"
    ...
    #   if [[ "$allow_gat_fallback" -eq 1 && -f "$nifa_tmpl" ]]; then
    #     echo "    -> fallback to gat tuned file: $nifa_tmpl"
    ...
    if [[ ! -f "$best_ea_path" ]]; then
      ...
      if [[ "$allow_gat_fallback" -eq 1 && -f "$ea_tmpl" ]]; then
        echo "    -> fallback to gat tuned file: $ea_tmpl"

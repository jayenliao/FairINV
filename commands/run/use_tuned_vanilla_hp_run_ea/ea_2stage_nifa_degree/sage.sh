...
echo "Running EdgeAdder under NIFA (degree) for 5 datasets x 1 backbone (sage) x 10 seeds..."
echo "CUDA_VISIBLE_DEVICES=$CUDA_VISIBLE_DEVICES"
echo "DRY_RUN=$DRY_RUN"
echo

# ---------------------------------------------
# Tuned best_overall.json paths (sage templates)
# We swap /sage/ -> /${encoder}/
# ---------------------------------------------
...
# -----------------------------
# Loops: sage x 5 datasets
# -----------------------------
for encoder in sage; do
  echo
  ...
  for dataset in pokec_z pokec_n nba bail german; do
    ...
    best_vanilla_path="${vanilla_tmpl/\/sage\//\/${encoder}\/}"
    # best_nifa_path="${nifa_tmpl/\/sage\//\/${encoder}\/}"
    best_ea_path="${ea_tmpl/\/sage\//\/${encoder}\/}"

    # -------- existence check + fallback to sage --------
    if [[ ! -f "$best_vanilla_path" ]]; then
      echo "⚠️  Warning: vanilla best_overall.json not found at: $best_vanilla_path"
      if [[ "$allow_sage_fallback" -eq 1 && -f "$vanilla_tmpl" ]]; then
        echo "    -> fallback to sage tuned file: $vanilla_tmpl"
    ...
    #   if [[ "$allow_sage_fallback" -eq 1 && -f "$nifa_tmpl" ]]; then
    #     echo "    -> fallback to sage tuned file: $nifa_tmpl"
    ...
    if [[ ! -f "$best_ea_path" ]]; then
      ...
      if [[ "$allow_sage_fallback" -eq 1 && -f "$ea_tmpl" ]]; then
        echo "    -> fallback to sage tuned file: $ea_tmpl"
